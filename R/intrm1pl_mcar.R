#' 1pl LSIRM model with multiplicative effect for missing completely at random data. 
#' 
#' @description \link{intrm1pl_mcar} is used to fit LSIRM model with 1pl using multiplicative effect in incomplete data under the missing completely at random assumption. 
#' \link{intrm1pl_mcar} factorizes item response matrix into column-wise item effect, row-wise respondent effect and further embeds multiplicative effect in a latent space, while ignoring the missing element under the assumption of missing completely at random. The resulting latent space provides an interaction map that represents interactions between respondents and items. 
#' 
...
#' 
#' 
#' @return \code{intrm1pl_mcar} returns an object of  list containing the following components:
...
#' 
#' lsirm_result <- intrm1pl_mcar(data)
#' 
#' # The code following can achieve the same result.
#' lsirm_result <- intrm1pl(data, missing_data = 'mcar')
#' }
#' 
#' @export
intrm1pl_mcar = function(data, ndim = 2, niter = 15000, nburn = 2500, nthin = 5, nprint = 500,
                    jump_beta = 0.4, jump_theta = 1, jump_delta = 1, 
                    pr_mean_beta = 0, pr_sd_beta = 1, pr_mean_theta = 0, 
                    pr_mean_delta = 0, pr_sd_delta = 1, pr_a_theta = 0.001, pr_b_theta = 0.001,
                    missing = 99){
  
  output <- intrm1pl_mcar_cpp(data=data, niter=niter, nburn=nburn, nthin=nthin, nprint=nprint, 
                      jump_beta=jump_beta, jump_theta=jump_theta, jump_delta=jump_delta, 
                      pr_mean_beta=pr_mean_beta, pr_sd_beta=pr_sd_beta, pr_mean_theta=pr_mean_theta,
                      pr_mean_delta=pr_mean_delta, pr_sd_delta=pr_sd_delta, pr_a_theta=pr_a_theta, pr_b_theta=pr_b_theta,
                      missing = missing)
