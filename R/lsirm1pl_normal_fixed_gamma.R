...
lsirm1pl_normal_fixed_gamma = function(data, ndim = 2, niter = 15000, nburn = 2500, nthin = 5, nprint = 500,
                                       ...
                                       pr_a_eps = 0.001, pr_b_eps = 0.001, verbose=FALSE, fix_theta_sd=FALSE, adapt = NULL) {
  if(niter <= nburn){
  ...
  output <- lsirm1pl_normal_fixed_gamma_cpp(data=as.matrix(data),  ndim=ndim,  niter=niter,  nburn=nburn,  nthin=nthin,  nprint=nprint,
                                            ...
                                            pr_a_theta=pr_a_theta, pr_b_theta=pr_b_theta,  pr_a_eps=pr_a_eps, pr_b_eps=pr_b_eps, verbose=verbose, fix_theta_sd=fix_theta_sd, adapt=adapt)
