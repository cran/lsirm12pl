#' 2pl LSIRM model fixing gamma to 1 for missing completely at random data. 
#' 
#' @description \link{lsirm2pl_fixed_gamma_mcar} is used to fit 2pl LSIRM model fixing gamma to 1 in incomplete data assumed to be missing completely at random. 
...
#' lsirm_result <- lsirm2pl(data, spikenslab = FALSE, fixed_gamma = TRUE, missing_data = "mcar")
#' }
#' 
#' @export
lsirm2pl_fixed_gamma_mcar = function(data, ndim = 2, niter = 15000, nburn = 2500, nthin = 5, nprint = 500,
  ...
  output <- lsirm2pl_fixed_gamma_mcar_cpp(data, ndim, niter, nburn, nthin, nprint,
