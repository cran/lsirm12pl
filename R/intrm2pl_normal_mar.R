#' 2pl LSIRM model with normal likelihood using multiplicative effect for missing at random data. 
#' 
#' @description \link{intrm2pl_normal_mar} is used to fit 2pl LSIRM model for continuous variable using multiplicative effect in incomplete data under missing at random assumption.
#' \link{intrm2pl_normal_mar} factorizes continuous item response matrix into column-wise item effect, row-wise respondent effect and further embeds multiplicative effect in a latent space, while considering the missing element under the assumption of missing at random. Unlike 1pl model, 2pl model assumes the item effect can vary according to respondent, allowing additional parameter multiplied with respondent effect. The resulting latent space provides an interaction map that represents interactions between respondents and items.
#' 
...
#' 
#' @return \code{intrm2pl_normal_mar} returns an object of  list containing the following components:
...
#' 
#' lsirm_result <- intrm2pl_normal_mar(data)
#' 
#' # The code following can achieve the same result.
#' lsirm_result <- intrm2pl_normal(data, missing_data = 'mar')
#' 
#' @export
intrm2pl_normal_mar = function(data, ndim = 2, niter = 15000, nburn = 2500, nthin = 5, nprint = 500,
  ...
  output <- intrm2pl_normal_mar_cpp(data, niter, nburn, nthin, nprint,
