#' 2pl LSIRM model fixing gamma to 1 with normal likelihood for missing completely at random data. 
#' 
#' @description \link{lsirm2pl_normal_fixed_gamma_mcar} is used to fit 2pl LSIRM model with gamma fixed to 1 for continuous variable in incomplete data assumed to be missing completely at random. 
#' 
...
#' lsirm_result <- lsirm2pl_normal(data, spikenslab = FALSE, fixed_gamma = TRUE,
#'                                 missing_data = "mcar")
#' 
#' 
#' @export
lsirm2pl_normal_fixed_gamma_mcar = function(data, ndim = 2, niter = 15000, nburn = 2500, nthin = 5, nprint = 500,
                          ...
                          pr_a_theta = 0.001, pr_b_theta = 0.001,pr_a_eps = 0.001, pr_b_eps = 0.001, missing = 99){
  
  output <- lsirm2pl_normal_fixed_gamma_mcar_cpp(data, 
  ...
  sigma_estimate = mean(output$sigma)
