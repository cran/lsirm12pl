#' 1PL GRM LSIRM for missing completely at random data.
#'
#' @description \link{lsirmgrm_mcar} is used to fit 1PL GRM LSIRM in incomplete data assumed to be missing completely at random.
#'
...
#'
#' # Fit 1PL GRM LSIRM with MCAR
#' fit <- lsirmgrm_mcar(data, niter = 1000, nburn = 500)
#' summary(fit)
#' }
#' 
#' @export
lsirmgrm_mcar <- function(data, ncat = NULL, missing.val = 99,
  ...
  lsirmgrm(data = data, ncat = ncat, missing_data = "mcar", missing.val = missing.val,
