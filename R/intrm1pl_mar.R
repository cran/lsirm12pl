#' 1pl LSIRM model using multiplicative effect for missing at random data. 
#' 
#' @description \link{intrm1pl_mar} is used to fit 1pl LSIRM model using multiplicative effect in incomplete data under missing at random assumption. 
#' \link{intrm1pl_mar} factorizes item response matrix into column-wise item effect, row-wise respondent effect and further embeds multiplicative effect in a latent space, while considering the missing element under the assumption of missing at random. The resulting latent space provides an interaction map that represents interactions between respondents and items. 
#' 
...
#' 
#' 
#' @return \code{intrm1pl_mar} returns an object of  list containing the following components:
...
#' 
#' lsirm_result <- intrm1pl_mar(data)
#' 
#' # The code following can achieve the same result.
#' lsirm_result <- intrm1pl(data, missing_data = 'mar')
#' }
#' 
#' @export
intrm1pl_mar = function(data, ndim = 2, niter = 15000, nburn = 2500, nthin = 5, nprint = 500,
  ...
  output <- intrm1pl_mar_cpp(data=data, niter=niter, nburn=nburn, nthin=nthin, nprint=nprint, 
