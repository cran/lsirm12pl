#' 1pl LSIRM model for missing completely at random data. 
#' 
#' @description \link{lsirm1pl_mcar} is used to fit 1pl LSIRM model in incomplete data assumed to be missing completely at random.
...
#' lsirm_result <- lsirm1pl(data, spikenslab = FALSE, fixed_gamma = FALSE, missing_data = 'mcar')
#' }
#' 
#' @export
lsirm1pl_mcar = function(data, ndim = 2, niter = 15000, nburn = 2500, nthin = 5, nprint = 500,
  ...
  output <- lsirm1pl_mcar_cpp(data=data, ndim=ndim, niter=niter, nburn=nburn, nthin=nthin, nprint=nprint,
