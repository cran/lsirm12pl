...
#' 
#' @description \link{intrm1pl_normal_o} is used to fit LSIRM model with 1pl for continuous variable using multiplicative effect.
#' \link{intrm1pl_normal_o} factorizes continuous item response matrix into column-wise item effect, row-wise respondent effect and further embeds multiplicative effect in a latent space. The resulting latent space provides an interaction map that represents interactions between respondents and items. 
#' 
...
#' 
#' 
#' @return \code{intrm1pl_normal_o} returns an object of  list containing the following components:
...
#'  \item{ls_mean_lambda}{posterior estimation lambda  which is singular value of decomposition.}
...
intrm1pl_normal_o = function(data, ndim = 2, niter = 15000, nburn = 2500, nthin = 5, nprint = 500,
  ...
  output <-  intrm1pl_normal_cpp(data, niter, nburn, nthin, nprint,
