#' 2pl LSIRM model fixing gamma to 1 with normal likelihood 
#' 
#' @description \link{lsirm2pl_normal_fixed_gamma} is used to fit 2pl LSIRM model with gamma fixed to 1 for continuous variable. 
...
lsirm2pl_normal_fixed_gamma = function(data, ndim = 2, niter = 15000, nburn = 2500, nthin = 5, nprint = 500,
  ...
  output <- lsirm2pl_normal_fixed_gamma_cpp(data, ndim, niter, nburn, nthin, nprint,
                            jump_beta, jump_theta, jump_alpha, jump_z, jump_w,
                            pr_mean_beta, pr_sd_beta, pr_a_theta, pr_b_theta,
                            pr_mean_theta,
                            pr_a_eps,  pr_b_eps, pr_mean_alpha, pr_sd_alpha)
