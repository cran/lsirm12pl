#' 1pl LSIRM model for missing at random data. 
#' 
#' @description \link{lsirm1pl_mar} is used to fit 1pl LSIRM model in incomplete data assumed to be missing at random.
...
#' lsirm_result <- lsirm1pl(data, spikenslab = FALSE, fixed_gamma = FALSE, missing_data ='mar')
#' }
#' 
#' @export
lsirm1pl_mar = function(data, ndim = 2, niter = 15000, nburn = 2500, nthin = 5, nprint = 500,
                       ...
                       pr_mean_gamma = 0.5, pr_sd_gamma = 1, pr_a_theta = 0.001, pr_b_theta = 0.001, missing = 99){
  
  output <- lsirm1pl_mar_cpp(data=data, ndim=ndim, niter=niter, nburn=nburn, nthin=nthin, nprint=nprint,
                         ...
                         pr_a_theta=pr_a_theta, pr_b_theta=pr_b_theta, missing = missing)
