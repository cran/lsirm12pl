...
lsirm1pl_fixed_gamma_mar = function(data, ndim = 2, niter = 15000, nburn = 2500, nthin = 5, nprint = 500,
                               ...
                               pr_mean_beta = 0, pr_sd_beta = 1.0, pr_mean_theta = 0, pr_a_theta = 0.001, pr_b_theta = 0.001, missing.val = 99, verbose=FALSE){
  if(niter < nburn){
  ...
  output <- lsirm1pl_fixed_gamma_mar_cpp(as.matrix(data), ndim, niter, nburn, nthin, nprint,
                           jump_beta, jump_theta, jump_z, jump_w,
                           pr_mean_beta, pr_sd_beta, pr_mean_theta, pr_a_theta, pr_b_theta, missing.val, verbose=verbose)
