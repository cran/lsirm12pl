...
#' 
#' @description \link{intrm2pl_o} is used to fit 2pl LSIRM model using multiplicative effect. 
#' \link{intrm2pl_o} factorizes item response matrix into column-wise item effect, row-wise respondent effect and further embeds multiplicative effect in a latent space. Unlike 1pl model, 2pl model assumes the item effect can vary according to respondent, allowing additional parameter multiplied with respondent effect. The resulting latent space provides an interaction map that represents interactions between respondents and items. 
#' 
...
#' 
#' 
#' @return \code{intrm2pl_o} returns an object of  list containing the following components:
...
#'  
#'  
#' @details \code{intrm2pl_o} models the probability of correct response by respondent \eqn{j} to item \eqn{i} with item effect \eqn{\beta_i}, respondent effect \eqn{\theta_j} in the shared metric space. For 2pl model, the the item effect is assumed to have additional discrimination parameter \eqn{\alpha_i} multiplied by \eqn{\theta_j} : \deqn{logit(P(Y_{j,i} = 1|\theta_j,\beta_i,\alpha_i,\delta_{j,i}))=\theta_j*\alpha_i+\beta_i+\delta_{j,i}} The final latent positions of respondents and items are the singular vectors of matrix with its \eqn{j,i} element \eqn{\delta_{j,i}}.
#' 
#' @examples 
#' \donttest{
...
#' 
#' lsirm_result <- intrm2pl_o(data)
#' 
#' # The code following can achieve the same result.
#' lsirm_result <- intrm2pl(data)
#' }
#' 
#' @export
intrm2pl_o = function(data, ndim = 2, niter = 15000, nburn = 2500, nthin = 5, nprint = 500,
  ...
  output <- intrm2pl_cpp(data, niter, nburn, nthin, nprint,
