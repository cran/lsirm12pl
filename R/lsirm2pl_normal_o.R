...
#' 
#' @description \link{lsirm2pl_normal_o} is used to fit 2pl LSIRM model for continuous variable. 
...
#' lsirm_result <- lsirm2pl_normal(data)
#' 
#' # The code following can achieve the same result.
...
lsirm2pl_normal_o = function(data, ndim = 2, niter = 15000, nburn = 2500, nthin = 5, nprint = 500,
  ...
  output <- lsirm2pl_normal_cpp(data, ndim, niter, nburn, nthin, nprint,
                            jump_beta, jump_theta, jump_alpha, jump_gamma, jump_z, jump_w,
                            pr_mean_beta, pr_sd_beta, pr_a_theta, pr_b_theta, pr_mean_theta,
                            pr_a_eps,  pr_b_eps, pr_mean_gamma, pr_sd_gamma, pr_mean_alpha, pr_sd_alpha)
