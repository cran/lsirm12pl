...
// [[Rcpp::export]]
Rcpp::List intrm2pl_mcar_cpp(arma::mat data, const int niter, const int nburn, const int nthin, const int nprint,
  ...
  for(int iter = 0; iter < niter; iter++){
    ...
    for(i = 1; i < nitem; i++){
      ...
      for(j = 0; j < nsample; j++){
        if(data(j,i) != missing){
          if(data(j,i) == 1.0) new_like_alpha += -std::log(1.0 + std::exp(-(oldbeta(i) + newalpha(i) * oldtheta(j) + olddelta(j,i))));
          else new_like_alpha += -std::log(1.0 + std::exp(oldbeta(i) + newalpha(i) * oldtheta(j) + olddelta(j,i)));
          if(data(j,i) == 1.0) old_like_alpha += -std::log(1.0 + std::exp(-(oldbeta(i) + oldalpha(i) * oldtheta(j) + olddelta(j,i))));
          else old_like_alpha += -std::log(1.0 + std::exp(oldbeta(i) + oldalpha(i) * oldtheta(j) + olddelta(j,i)));
        }
      }
