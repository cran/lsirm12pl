...
// [[Rcpp::export]]
Rcpp::List intrm1pl_mar_cpp(arma::mat data, 
                  ...
                  const double pr_mean_delta, const double pr_sd_delta, const double pr_a_theta, const double pr_b_theta, const double missing){
  ...
  for(int iter = 0; iter < niter; iter++){
    ...
    for(i =0; i<nitem; i++){
      for(k =0; k<nsample; k++){
        if(missing_data(k,i) == missing){
          p_ki = 1 / (1 + std::exp(-(oldbeta(i) + oldtheta(k) + olddelta(k,i))));
