...
// [[Rcpp::export]]
Rcpp::List intrm1pl_normal_mcar_cpp(arma::mat data, const int niter, const int nburn, const int nthin, const int nprint,
  ...
  for(int iter = 0; iter < niter; iter++){
    ...
    for(j = 0; j < nsample; j++){
      for(i = 0; i < nitem; i++){
        if(data(j,i) != missing){
          post_b_sigma += std::pow((data(j,i) - oldbeta(i) - oldtheta(j) - olddelta(j,i)), 2.0) / 2; 
        }
      }
    }
