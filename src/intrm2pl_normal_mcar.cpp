...
// [[Rcpp::export]]
Rcpp::List intrm2pl_normal_mcar_cpp(arma::mat data, 
                                    ...
                                    const double pr_mean_delta, const double pr_sd_delta, const double pr_mean_alpha, const double pr_sd_alpha, const double missing){
  
  int i, j, count, accept, num_notmissing;
  ...
  for(int iter = 0; iter < niter; iter++){
    ...
    for(j = 0; j < nsample; j++){
      for(i = 0; i < nitem; i++){
        if(data(j,i) != missing){
          post_b_sigma += std::pow((data(j, i) - oldbeta(i) - oldalpha(i) * oldtheta(j) - olddelta(j, i)), 2.0) /2;
        }
      }
    }
